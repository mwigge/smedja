...
When tasks fan out to parallel worktrees, agents share **read** access to `smedja-vault` (cold store) but write to isolated working trees. The orchestrator merges vault writes on task completion. Cold retrieval is wired end-to-end: each turn the orchestrator recalls relevant context from the vault and injects it as a bounded `<cold_context>` block, and the `smedja_vault_search` tool embeds the query, runs hybrid cosine + keyword + recency search over the named namespace, and returns ranked results (an empty `results` array on no match). Retrieval is **semantic by default**: the bundled local model (all-MiniLM / bge-small, 384-dim) is downloaded on first use into `~/.local/share/smedja/models/` and serves embeddings out of the box with no sidecar server. If the model cannot be fetched or loaded (offline, uncached, or compiled without the `local-embedder` feature), the daemon degrades to a deterministic FNV bag-of-words hash and logs the degradation so lexical-only recall is never silent. An external OpenAI-compatible `/v1/embeddings` endpoint can also be configured via `[embedder]` in `.smedja/config.toml`; that learned path degrades to the same FNV hash if its endpoint is unreachable.
...
> **Naming note:** smedja's **ACP** is its own **Agent *Coordination* Protocol** — an optional HTTP session API (`/acp/v1/…`, enabled by `SMEDJA_ACP_PORT`). It is distinct from the industry **Agent *Client* Protocol** (the Zed/JetBrains editor↔agent standard): smedja does not expose an industry ACP *server*, but it **does** implement an ACP *client* that drives external agent CLIs such as `kimi acp` and `gemini --acp` over stdio JSON-RPC and gates their tool calls through the same approval path as in-process tools.

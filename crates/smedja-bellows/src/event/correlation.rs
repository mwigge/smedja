...
// ── Tests ─────────────────────────────────────────────────────────────────────
//
// These exercise how `CorrelationCtx` behaves on the wire — flattening,
// omission of `None` fields, and roundtripping — using `TurnEvent` variants
// as the carrier, since the context is only ever serialised embedded in one.

#[cfg(test)]
mod tests {
    use super::CorrelationCtx;

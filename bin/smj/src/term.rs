//! `smj term` — smedja utilities (binary install, config conversion).
...
/// Dispatches a `smj term` subcommand.
pub(crate) async fn run(action: TermCmd) -> Result<()> {
    match action {
...
fn cmd_term_install(url: &str, prefix: &Path) -> Result<()> {

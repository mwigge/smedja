...
/// (idempotency guard).
fn hook_install(kind: ShellKind, path: &std::path::Path) -> anyhow::Result<()> {
...
async fn cmd_tool_gate(sock: &std::path::Path) {
            ...
            ShellCmd::Hook {
                ...
                let config_path = match kind {
                    ShellKind::Fish => {
                        ...
                        std::path::PathBuf::from(config_home).join("fish/config.fish")
                    }
                    ShellKind::Zsh => std::path::PathBuf::from(
                        std::env::var("HOME").unwrap_or_else(|_| ".".into()),
                    )
                    .join(".zshrc"),
                    ShellKind::Bash => std::path::PathBuf::from(
                        std::env::var("HOME").unwrap_or_else(|_| ".".into()),
                    )
                    .join(".bashrc"),
                };
...
async fn cmd_doctor(sock: &std::path::Path, json: bool) -> Result<()> {
...
async fn connect_or_exit(sock: &std::path::Path) -> Client {
    match Client::connect(sock).await {
        Ok(c) => c,
        Err(e) => {
...
fn find_gov_artifact(gov_dir: &std::path::Path, id: &str) -> Option<std::path::PathBuf> {

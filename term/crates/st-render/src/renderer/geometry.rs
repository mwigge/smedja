//! Vertex and geometry builders for [`Renderer`].
//!
//! NDC coordinate math plus emission of background, glyph, and colour-glyph
//! quads. These are private helpers on [`Renderer`]; the three `build_*`
//! entry points are `pub(super)` so the parent render loop can call them.

use crate::atlas::ATLAS_SIZE;
use crate::renderer::Renderer;
use crate::vertex::{BgVertex, GlyphVertex};
...
impl Renderer {
    ...
    pub(super) fn build_bg_vertices(&self) -> Vec<BgVertex> {
    ...
    pub(super) fn build_glyph_vertices(&self) -> Vec<GlyphVertex> {
        let (cw, ch) = self.cell_size();
    ...
    pub(super) fn build_colour_glyph_vertices(&self) -> Vec<GlyphVertex> {
        let (cw, ch) = self.cell_size();
        let atlas_size_f = ATLAS_SIZE as f32;
        let top_off = self.top_bar_height_px() as f32;
